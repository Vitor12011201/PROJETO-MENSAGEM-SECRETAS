...
export function AdminPageClient() {
  ...
  return (
    <main className="mx-auto max-w-7xl space-y-6 px-4 py-8 sm:px-6 lg:px-8">
      <div><p className="text-sm text-lilac">Admin</p><h1 className="text-3xl font-bold">Painel administrativo</h1></div>
      ...
      <section className="grid gap-5 lg:grid-cols-2">
        <Card>
          ...
          <div className="mt-4 space-y-3">{state.messages.filter((message) => message.moderationStatus === "needs_review").map((message) => <div key={`${message.id}-${version}`} className="rounded-lg border border-white/10 p-3"><p className="text-sm text-slateText">Score {message.moderationScore}  {message.moderationReasons.join(", ")}</p><p className="mt-2">{message.content}</p><div className="mt-3 flex gap-2"><Button onClick={() => refresh(() => reviewMessage(message.id, "approved"))}><Check size={16} /> Aprovar</Button><Button variant="danger" onClick={() => refresh(() => reviewMessage(message.id, "rejected"))}><Trash2 size={16} /> Remover</Button></div></div>)}</div>
        </Card>
        <Card>
          <h2 className="text-xl font-bold">Denúncias abertas</h2>
          <div className="mt-4 space-y-3">{state.reports.filter((report) => report.status === "open").map((report) => <div key={`${report.id}-${version}`} className="rounded-lg border border-white/10 p-3"><p className="text-sm text-slateText">{report.reason}</p><p className="mt-2">Mensagem: {report.messageId}</p><Button className="mt-3" variant="secondary" onClick={() => refresh(() => resolveReport(report.id))}><ShieldAlert size={16} /> Marcar como resolvida</Button></div>)}</div>
        </Card>
      </section>
